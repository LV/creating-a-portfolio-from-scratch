...

{
  ...
  return html`<div>
    <p style="font-size:15px; color:#444; font-weight:600; margin:0 0 2px;">${titleText}</p>
    ${sharpeEl}
    ${plot}
  </div>`;
}
...
### B: Why portfolios of lowly correlated assets are better {#sec-diversification}

In the following examples, we'll illustrate why combining two assets that do not correlate together, given positive returns, can result in a significantly more attractive portfolio
...
Let $\text{Correlation}_{A,B} = r_{A,B} = 1$

This means that the graphs could look something like this:
...
<div style="background:#fff;border:1px solid #ddd;border-radius:4px;padding:20px 24px 14px;margin:16px 0;font-family:'Arial Black',sans-serif;">
  <div style="font-size:16px;font-weight:900;text-transform:uppercase;letter-spacing:0.5px;line-height:1.3;margin-bottom:12px;color:#1a1a2e;">
    PORTFOLIO COMPOSED OF ASSETS WITH <span style="color:#E8A020;">SIMILAR BEHAVIOR</span>
  </div>
  <svg id="corr-chart" viewBox="0 0 720 260" style="display:block;width:100%;background:#fafafa;border:1px solid #e8e8e8;"></svg>
  <div style="display:flex;gap:20px;margin-top:10px;flex-wrap:wrap;align-items:center;font-family:Georgia,serif;font-size:13px;">
    ...
    <div style="margin-left:auto;color:#888;font-style:italic;">Correlation: <strong style="color:#1a1a2e;">≈ +1.00</strong></div>
  </div>
</div>
<script>
(function(){
  ...
  render('corr-chart', wave(1.4,0,0.8,0.4,0.15), wave(1.4,0.18,0.8,0.4,0.15));
})();
</script>
```
...
<div style="background:#fff;border:1px solid #ddd;border-radius:4px;padding:20px 24px 14px;margin:16px 0;font-family:'Arial Black',sans-serif;">
  <div style="font-size:16px;font-weight:900;text-transform:uppercase;letter-spacing:0.5px;line-height:1.3;margin-bottom:12px;color:#1a1a2e;">
    PORTFOLIO COMPOSED OF ASSETS WITH <span style="color:#E8A020;">NEGATIVELY CORRELATED BEHAVIOR</span>
  </div>
  <svg id="neg-chart" viewBox="0 0 720 260" style="display:block;width:100%;background:#fafafa;border:1px solid #e8e8e8;"></svg>
  <div style="display:flex;gap:20px;margin-top:10px;flex-wrap:wrap;align-items:center;font-family:Georgia,serif;font-size:13px;">
    <div style="display:flex;align-items:center;gap:6px;color:#444;"><div style="width:24px;height:3px;background:#E8A020;border-radius:2px;"></div>Asset A</div>
    <div style="display:flex;align-items:center;gap:6px;color:#444;"><div style="width:24px;height:3px;background:#CC2233;border-radius:2px;"></div>Asset B</div>
    <div style="display:flex;align-items:center;gap:6px;color:#222;font-weight:700;"><div style="width:28px;height:4px;background:#38C8E8;border-radius:2px;"></div>Portfolio (equal weight)</div>
...
(function(){
  ...
  render('neg-chart', wave(1.4,0,0.8,0.4,0.15), wave(1.4,Math.PI,0.8,0.4,0.15));
})();
</script>
```
